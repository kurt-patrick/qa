...
{
    public class RegistrationTests : TestFixtureBase
    {
        ...
        public RegistrationTests(TestFixtureConfig config) 
            : base(config) 
        { 
        }
        ...
        private void SetPageValues(RegistrationDs dsRow)
        {
            ...
            // Press tab after entering text into (password confirm) - to fire off validation
            //KeyboardHelper.Create(_driver).Tab();
        ...
        {
            return DataSetHelper.LoadFromCsv<RegistrationDs>(@"Tests\Functional\Registration\Datasets\Registration.csv");
        }
